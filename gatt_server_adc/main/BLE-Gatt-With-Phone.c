...
#define GPIO_OUTPUT_PIN_SEL (1ULL << POWER_SUPPLY_PIN)
...
void adc_setup()
{
    ...
    gpio_set_level(POWER_SUPPLY_PIN, 1);
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        float v = ((float) adc_reading) / 4096.0 * 3.9;
        float old_v = v;
        v = v + 1.99 - 0.06;
        float PPM =  (150.4351049*v*v*v*v*v) - (2244.75988*v*v*v*v) + (13308.5139*v*v*v) - (39136.08594*v*v) + (57082.6258*v) - 32982.05333;
        printf("Raw: %d\t Voltage: %f V\t PPM: %f\t BAC: %f\t \n", adc_reading, old_v, PPM, PPM / 2600.0);
        int BAC = (int) (PPM/2600 * 1000);
        rsp.attr_value.handle = param->read.handle;
        rsp.attr_value.len = 4;
        rsp.attr_value.value[0] = 0x00;
        rsp.attr_value.value[1] = 0x00;
        rsp.attr_value.value[2] = (BAC < 0) ? 1 : ((BAC > 150) ? 2 : 0);
        rsp.attr_value.value[3] = BAC & ((1 << 8) - 1);
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        // v = v + 1.99 - 0.2;
        // float PPM =  (150.4351049*v*v*v*v*v) - (2244.75988*v*v*v*v) + (13308.5139*v*v*v) - (39136.08594*v*v) + (57082.6258*v) - 32982.05333;
        // printf("Raw: %d\t Voltage: %f mV\t PPM: %f\t BAC: %f\t \n", adc_reading, v, PPM, PPM / 2600.0);
        ...
        rsp.attr_value.value[2] = (adc_reading >> 8) & ((1 << 8) - 1);
        rsp.attr_value.value[3] = adc_reading & ((1 << 8) - 1);
        esp_ble_gatts_send_response(gatts_if, param->read.conn_id, param->read.trans_id,
                                    ESP_GATT_OK, &rsp);
        break;
    }
